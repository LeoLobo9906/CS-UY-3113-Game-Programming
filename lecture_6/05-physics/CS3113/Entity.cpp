...
void Entity::checkCollisionY(Entity *collidableEntities, int collisionCheckCount)
{
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        {
            if (mEntityType == PLAYER && collidableEntity->mEntityType == BLOCK)
            {
...
void Entity::update(float deltaTime, Entity *tiles, int tileCount, Entity *blocks, int blockCount)
{
    ...
    if (tiles)  checkCollisionY(tiles, tileCount);
    if (blocks) checkCollisionY(blocks, blockCount);
