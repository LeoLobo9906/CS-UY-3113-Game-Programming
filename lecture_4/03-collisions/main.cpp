...
void processInput()
{
    ...
    // TODO - Get mouse position gMousePosition
    gMousePosition=GetMousePosition();
    ...
    if(IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
        if (isColliding( &gMousePosition,  &gMouseScale, &gTeardropPosition, &gScale)) {
            printf("Mouse Collision \n");
...
void update()
{
    ...
    if(gTeardropStatus == RELEASED) {
        ...
        gTeardropPosition.y = gTeardropPosition.y + SPEED * 1 * deltaTime;
    }
    else{
    ...
    if (isColliding(&gBeakerPosition, &gBeakerScale,
                        &gTeardropPosition, &gScale))
    {
        printf("Beaker Collision\n");

        gScale.x -= SHRINK_RATE * deltaTime;
        gScale.y -= SHRINK_RATE * deltaTime;

        if (gScale.x < 0.0f) gScale.x = 0.0f;
        if (gScale.y < 0.0f) gScale.y = 0.0f;
    }
}
